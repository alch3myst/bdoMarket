...
function buildCookingRecipes()
{
    ...
    // Reset Table
    $db->exec("DROP TABLE cookingRecipes");

    // Create the values table
    $db->exec("CREATE TABLE IF NOT EXISTS cookingRecipes(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name TEXT, cost INTEGER, price INTEGER, rare_price INTEGER, recipe TEXT, canProc TEXT)");

    // Get the json file content
    $file_path = "../../src/recipes/cooking.json";
    ...
    foreach ($json_file["allrecipes"] as $recipes => $recipie) {
        ...
        $main_query = $db->prepare("INSERT INTO cookingRecipes(name, cost, price, rare_price, recipe, canProc) VALUES(?, ?, ?, ?, ?, ?)");
