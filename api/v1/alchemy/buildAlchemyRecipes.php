...
function buildCookingRecipes()
{
    ...
    // Reset Table
    $db->exec("DROP TABLE alchemyRecipes");

    // Create the values table
    $db->exec("CREATE TABLE IF NOT EXISTS alchemyRecipes(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name TEXT, cost INTEGER, price INTEGER, rare_price INTEGER, recipe TEXT, canProc TEXT)");

    // Get the json file content
    $file_path = "../../src/recipes/alchemy.json";
    $json_file = json_decode(file_get_contents($file_path), true);

    // For each recipe on alchemy.json
    foreach ($json_file["allrecipes"] as $recipes => $recipie) {

        // Make insert query on alchemyRecies table
        $main_query = $db->prepare("INSERT INTO alchemyRecipes(name, cost, price, rare_price, recipe, canProc) VALUES(?, ?, ?, ?, ?, ?)");
        ...
        if (sizeof($recipie["recipe"]) > 1) {
            ...
            // Can proc?
            if(isset($recipie["canProc"])){
            $main_query->bindValue(6, $recipie["canProc"]);
            } else {
                $main_query->bindValue(6, "true");
            }
