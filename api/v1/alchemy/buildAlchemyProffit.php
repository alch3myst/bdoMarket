...
function buildAlchemyProffit()
{
    // "Connect to db"
    $db = new SQLite3("../../src/db/bdomarket.sqlite");

    $db->query("DROP TABLE alchemyProffit");
    $db->query("CREATE TABLE IF NOT EXISTS alchemyProffit (
    ...
    try {
        // Get all recipes
        $recipesQuery = $db->query("SELECT * FROM alchemyRecipes");
        ...
        while ($recipesQueryResult = $recipesQuery->fetchArray(SQLITE3_ASSOC)) {
            ...
            foreach ($ingredients as $inkey => $ingredient) {
                ...
                // Try to get the recipe from alchemyRecipes table
                $ingredientQuery = $db->prepare("SELECT * FROM alchemyRecipes WHERE name = ?");
            ...
            $insertRecipeProffit = $db->prepare("INSERT INTO alchemyProffit (name, marketPrice, rareProcMarketPrice, recipe, cost, proffitWPE, proffitWTPE) values(?, ?, ?, ?, ?, ?, ?)");
            // Recipe name
...
function calc_proffit_cpe($cost, $price, $rare_price, $canProc, $normalProc = 2.64, $rareProc = 0.3)
{
    if ($canProc == "true") {
        $profit = ((($price * $normalProc) + ($rare_price * $rareProc)) * 0.845) - $cost;
    } else if($canProc == "special") {
        $profit = ((($price * $normalProc / 2) + ($rare_price * $rareProc / 2)) * 0.845) - $cost;
    } else {
...
function calc_proffit_spe($cost, $price, $rare_price, $canProc, $normalProc = 2.64, $rareProc = 0.3)
{
    if ($canProc == "true") {
        $profit = ((($price * $normalProc) + ($rare_price * $rareProc)) * 0.65) - $cost;
    } else if($canProc == "special") {
        $profit = ((($price * $normalProc / 2) + ($rare_price * $rareProc / 2)) * 0.65) - $cost;
    } else {
