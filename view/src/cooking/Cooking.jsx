...
class Cooking extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("http://localhost:9988/api/v1/cooking/getCookingProffit.php")
      .then((res) => {
        this.setState({ cookingProffit: res.data });
      });
  }
  ...
  search() {
    if (this.state.searchfor !== "") {
      axios
        .get(
          "http://localhost:9988/api/v1/cooking/searchCookingProffit.php?recipe=" +
    ...
    return (
      <div>
        ...
        <div className="cooking-list">
          {this.state.cookingProffit.map((recipe) => (
            <ul key={recipe.name}>
              {/* Recipe name */}
              <li>
                <b>{recipe.name}</b>
              ...
              <li className="recipe-li">
                {JSON.parse(recipe.recipe).map((ing) => (
                  ...
                  */
                  <div className="m5-tb" key={"recipe_ingredient_"+Math.random()}>
                    <p>
                      {ing[0]} * {ing[1]}
                    </p>
                    <p>
