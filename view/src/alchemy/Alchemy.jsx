...
class Alchemy extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("http://localhost:9988/api/v1/Alchemy/getAlchemyProffit.php")
      .then((res) => {
        this.setState({ AlchemyProffit: res.data });
      });
  }
  ...
  search() {
    if (this.state.searchfor !== "") {
      axios
        .get(
          "http://localhost:9988/api/v1/Alchemy/searchAlchemyProffit.php?recipe=" +
    ...
    return (
      <div>
        ...
        <div className="alchemy-list">
          {this.state.AlchemyProffit.map((recipe) => (
            <ul key={recipe.name}>
              {/* Recipe name */}
              <li>
                <b>{recipe.name}</b>
